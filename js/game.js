...
function getResetGain(layer, useType = null) {
	...
	if (type == "static") {
		...
		val = gain.floor().sub(player[layer].points).add(1).max(1);
	} else if (type == "normal") {
		...
		gain = gain.times(tmp[layer].directMult)
		val = gain.floor().max(0);
        
        if (tmp[layer].passiveGeneration === true) {
            var realGain = val
            var realGain100 = val.mul(100)
            if (player[layer].points.gte(realGain100)) {
                var ratio = player[layer].points.div(realGain100).max(1)
...
function generatePoints(layer, diff) {
    // Modified by Seako
    // 2024.8.1, added penalty over 100x production, same as getPointGain in mod.js
    
    var realGain = diff
    /*
    var realGain100 = tmp[layer].resetGain.mul(100)
    if (player[layer].points.gte(realGain100)) {
        var ratio = player[layer].points.div(realGain100).max(1).toNumber()
        realGain = realGain / ratio / Math.max((Math.log(ratio) / Math.log(1.13), 1)) * (2 - ratio)
        realGain = Math.min(realGain, diff)
        realGain = Math.max(realGain, 0)
    }
    */
	addPoints(layer, tmp[layer].resetGain.times(realGain))
}
...
function doReset(layer, force=false) {
    ...
    if (tmp[layer].autoPrestige != true)
